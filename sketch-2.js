...
const colors = [col12, col13, col14, col15];
exporters_1 = shuffle(exporters);
let col1 = exporters[1];
col1 = shuffle(col1);
console.log(col1);
let col11 = col1[0];
console.log(col11);
...
const sketch = () => {
  ...
  return ({ context, width, height }) => {
    ...
    context.fillStyle = "hsl(0, 0%, 99%)";
    context.fillStyle = col11;
    context.fillRect(0, 0, width, height);

    // All units are in 'cm'
    context.lineWidth = 5 * random.value();

    // Draw each point
    points.forEach(({ position, size }) => {
      const radius = size * width * 0.005;
      const [u, v] = position;

      // Apply margin to UV coordinates
      const x = lerp(margin, width - margin - 25, u);
      const y = lerp(margin, height - margin - 35, v);

      // Draw our shapes

      context.lineWidth = 50 * random.value();
      context.fillStyle = "hsl(0, 0%, 55%)";
      context.fillStyle = colors[n];
      context.beginPath();
      let sizelo = random.value(width) * 0.1;
      //context.arc(x, y, radius, 0, Math.PI * 3, false);
      //context.strokeStyle = colors[n];
      //context.stroke();
      //context.rect(x * random.value(width), y * random.value(height), x, y);
      //context.rect(0 * random.value(width), y * random.value(height), width, y);
      context.rect(x, y + sizelo, x + sizelo, y);
      context.fill();
